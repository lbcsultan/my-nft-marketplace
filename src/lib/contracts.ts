...
export async function setApprovalForAllNFT(spender: `0x${string}`, approved: boolean) {
  const hash = await writeContract(wagmiConfig, {
    address: nftContractAddress as `0x${string}`,
    abi: nftAbi,
...
export async function getTokenAllowance(owner: `0x${string}`, spender: `0x${string}`): Promise<bigint> {
  const result = await readContract(wagmiConfig, {
    address: tokenContractAddress as `0x${string}`,
    abi: tokenAbi,
