...
import { marketplaceContractAddress, tokenContractAddress } from '@/lib/constants'
...
export function Marketplace() {
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      const listedTokens: Array<{ tokenId: bigint; listing: Awaited<ReturnType<typeof getListing>> }> = []
      ...
      const listingPromises = listedTokens.map(async ({ tokenId, listing }): Promise<NFTListing> => {
  ...
  const fetchTokenInfo = async () => {
    ...
    try {
      ...
      console.log('토큰 정보:', { decimals, symbol, balance: balance.toString() })
    } catch (error: any) {
      console.error('토큰 정보 조회 오류:', error)
      setBalanceError(`토큰 정보 조회 실패: ${error.message || '알 수 없는 오류'}`)
    }
  }
  ...
  const handleBuy = async (tokenId: bigint, price: bigint) => {
    ...
    try {
      // 1. 토큰 승인 확인 및 처리
      const allowance = await getTokenAllowance(address, marketplaceContractAddress as `0x${string}`)
      if (allowance < price) {
        setStatus('토큰 승인 중...')
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-start mb-4">
            <h2 className="text-2xl font-bold">{listing.name || `NFT #${listing.tokenId.toString()}`}</h2>
            <button
              onClick={onClose}
          ...
          <div className="space-y-4 mb-6">
            <div>
              <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Token ID</h3>
              <p className="text-lg font-mono">{listing.tokenId.toString()}</p>
            </div>

            <div>
              <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">가격</h3>
            ...
            {details?.owner && (
              <div>
                <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">현재 소유자</h3>
            ...
            {listing.seller && (
              <div>
                <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">판매자</h3>
                <div className="flex items-center gap-2">
                  <code className="text-sm font-mono bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded">
                    {listing.seller}
                  </code>
          ...
          {details?.description && (
            <div className="mb-6">
              <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">설명</h3>
          ...
          {details?.attributes && details.attributes.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">속성</h3>
